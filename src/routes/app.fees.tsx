...
import { 
  DollarSign, TrendingUp, AlertCircle, Wallet, Send, CreditCard, 
  CheckCircle2, Clock, AlertTriangle, ChevronRight, Zap, Download, 
  FileText, Bell, Eye
} from "lucide-react";
...
function AdminFeesView() {
  ...
  const kpiData = useMemo(() => {
    if (period === "month") {
      return {
        totalCollection: { value: "₹4.8L", change: "+8% vs last month", desc: "Total fees collected this month" },
        targetAchievement: { value: "82%", change: "On track", desc: "Progress towards monthly target" },
        outstandingDues: { value: "₹14.2L", change: "Pending", desc: "Total outstanding fees across school" },
        defaulters: { value: "84", change: "Students", desc: "Students with pending fees" },
        expectedCollection: { value: "₹5.4L", change: "End of month forecast", desc: "Estimated collection by month-end" },
        healthScore: { value: "87/100", change: "Healthy", desc: "Overall financial health score" }
      };
    } else {
      return {
        totalCollection: { value: "₹68.4L", change: "+12% vs last year", desc: "Total fees collected this year" },
        targetAchievement: { value: "92%", change: "Exceeding", desc: "Annual collection target progress" },
        outstandingDues: { value: "₹8.6L", change: "Year-to-date", desc: "Outstanding fees pending collection" },
        defaulters: { value: "84", change: "Students", desc: "Active defaulters in school" },
        expectedCollection: { value: "₹77L", change: "Year-end forecast", desc: "Estimated total collection by year-end" },
        healthScore: { value: "89/100", change: "Very Healthy", desc: "Annual financial health score" }
      };
    }
  }, [period]);

  const financialMetrics = useMemo(() => [
    { id: "collection", label: "Collection Health", value: 87, status: "Healthy", description: "82% of monthly target achieved. Collection performance is on track." },
    { id: "recovery", label: "Fee Recovery", value: 76, status: "Good", description: "76% of total fees recovered. Outstanding dues are manageable." },
    { id: "defaulter", label: "Defaulter Risk", value: "High", status: "24 students", description: "24 students have overdue fees exceeding 60 days. Immediate follow-up required." },
    { id: "cashflow", label: "Cash Flow Status", value: "Stable", status: "On Track", description: "Monthly cash flow is stable with no critical gaps." },
    { id: "compliance", label: "Payment Compliance", value: 92, status: "Excellent", description: "92% of students maintain payment schedules on time." },
  ], []);
  ...
  const defaulterRisks = [
    { level: "High Risk", count: 24, days: "Overdue > 60 Days", amount: "₹8.2L", color: "bg-red-50 border-red-200" },
    { level: "Medium Risk", count: 38, days: "Overdue 30–60 Days", amount: "₹4.8L", color: "bg-amber-50 border-amber-200" },
    { level: "Low Risk", count: 22, days: "Overdue < 30 Days", amount: "₹1.2L", color: "bg-yellow-50 border-yellow-200" },
  ];

  const recentPayments = [
    { name: "Rahul Sharma", amount: "₹12,000", date: "Today", class: "Grade 4" },
    { name: "Priya Singh", amount: "₹18,500", date: "Yesterday", class: "Grade 5" },
    { name: "Aditya Kumar", amount: "₹15,000", date: "2 days ago", class: "Grade 3" },
    { name: "Maya Patel", amount: "₹22,000", date: "3 days ago", class: "Grade 6" },
    { name: "Vivaan Verma", amount: "₹14,200", date: "3 days ago", class: "Grade 2" },
  ];

  const paymentStatus = [
    { status: "Paid", count: 340, color: "text-emerald-600", bg: "bg-emerald-50 border-emerald-200" },
    ...
    <div>
      <PageHeader 
        ...
        action={
          <div className="flex items-center gap-2">
            <Button size="sm" variant="outline" onClick={() => toast.success("Payment recorded (demo)")}>
              <DollarSign className="size-4 mr-2" />Record Payment
            </Button>
            <Button size="sm" variant="outline" onClick={() => toast.success("Reminder sent (demo)")}>
              <Bell className="size-4 mr-2" />Send Fee Reminder
            </Button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 mb-6">
        ...
        <Card className="p-4">
          ...
          <div className="text-xs text-muted-foreground mt-2">{kpiData.expectedCollection.desc}</div>
        </Card>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          {financialMetrics.map((metric) => (
            <Card 
              ...
              onClick={() => setExpandedHealthMetric(expandedHealthMetric === metric.id ? null : metric.id)}
            >
              ...
              {expandedHealthMetric === metric.id && (
      ...
      <Card className="p-5 mb-6">
        <h2 className="text-lg font-semibold mb-4">Collection by Grade</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
          {classCollection.map((cls) => (
            <div key={cls.name} className="p-4 rounded-lg border border-border hover:border-muted-foreground transition-colors">
              <p className="font-semibold text-sm mb-2">{cls.name}</p>
              <p className="text-2xl font-bold mb-2">{cls.collected}%</p>
              <Badge 
                variant="outline" 
                className="text-xs"
                style={{
                  backgroundColor: cls.status === "Excellent" ? "#ecfdf5" : cls.status === "Good" ? "#f0f9ff" : "#fffbeb",
                  color: cls.status === "Excellent" ? "#047857" : cls.status === "Good" ? "#0369a1" : "#b45309",
                  borderColor: cls.status === "Excellent" ? "#d1fae5" : cls.status === "Good" ? "#bfdbfe" : "#fef08a",
                }}
              >
                {cls.status}
              </Badge>
      ...
      {/* DEFAULTER RISK SECTION */}
      <Card className="p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold">Defaulter Risk Analysis</h2>
          <Button size="sm" variant="outline">View All Students</Button>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {defaulterRisks.map((risk) => (
            <div key={risk.level} className={`p-4 rounded-lg border ${risk.color}`}>
              <p className="font-semibold text-sm mb-1">{risk.level}</p>
              <p className="text-3xl font-bold mb-2">{risk.count}</p>
              <p className="text-xs text-muted-foreground mb-2">{risk.days}</p>
              <p className="text-sm font-semibold">{risk.amount}</p>
            </div>
          ))}
        </div>
      </Card>
              ...
              <div key={idx} className="p-4 rounded-lg border border-border hover:bg-muted/50 transition-colors">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">{payment.class} • {payment.date}</p>
                  </div>
      ...
      <Card className="p-5 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 mb-6">
        ...
        <p className="text-sm text-slate-700 leading-relaxed">
          Collection performance remains healthy this month with 82% of targets achieved and ₹68.4L collected. Outstanding dues are concentrated in Grades 3 and 6, requiring targeted follow-up. Current trends suggest monthly targets are achievable by month-end with focused collection efforts.
        </p>
      </Card>
      ...
      <Card className="p-5">
        <div className="flex items-end gap-3 flex-wrap">
          <div className="flex-1 min-w-60">
            <label className="text-xs font-medium text-muted-foreground mb-2 block">Search Student</label>
            <Input 
